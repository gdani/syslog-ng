...
static void
vp_merge_set_impl(ValuePairs *vp, LogMessage *msg, gint32 seq_num, ValuePairSpec *set, GTree *dest, const LogTemplateOptions *template_options, gboolean exclude)
{
  gint i;
  SBTHGString *sb;
...
static void
vp_merge_macros(ValuePairs *vp, LogMessage *msg, gint32 seq_num, GTree *dest, const LogTemplateOptions *template_options)
{
  vp_merge_set_impl(vp, msg, seq_num, all_macros, dest, template_options, FALSE);
}
...
{
  vp_merge_set_impl(vp, msg, seq_num, set, dest, template_options, TRUE);
}
